sudo pacman -S --needed git base-devel && git clone https://aur.archlinux.org/yay.git && cd yay && makepkg -si && cd ~ && yay -S --needed --noconfirm gvfs thunar-archive-plugin thunar-media-tags-plugin thunar-volman \
        ...
        vulkan-icd-loader lib33-vulkan-icd-loader obs-studio discord mangohud lib32-mangohud goverlay gamescope bluez bluez-utils lib32-libpulse pipewire pipewire-pulse pipewire-alsa linux-headers xwaylandvideobridge \
        polychromatic openrazer-daemon opentabletdriver cosu-trainer-bin \
        pamac-all networkmanager && sudo usermod -aG gamemode damieon && sudo systemctl enable fstrim.timer && sudo systemctl enable NetworkManager.service && sudo systemctl enable bluetooth.service && sudo systemctl enable cronie.service && sudo systemctl enable lidm.service && sudo modprobe razermouse && sudo gpasswd -a $USER plugdev && sudo systemctl enable --now lactd && echo /usr/bin/fish | sudo tee -a /etc/shells && chsh -s /usr/bin/fish && fish
